...
- You can pass in the `link_list_resources` as `false` to avoid linking adding the Group and Linkage List resource being linked to the `list_resource_id`.  This is mostly suitable when syncing with `Location` and `RelatedEntityLocation` sync strategy
