...
import org.smartregister.structuremaptool.extensions.*
...
fun getQuestionsPath(questionnaire: Questionnaire): HashMap<String, String> {
  ...
  questionnaire.item.forEach { itemComponent -> getQuestionNames("", itemComponent, questionsMap) }
  return questionsMap
}
...
class Group(
  ...
  fun generateGroup(questionnaireResponse: QuestionnaireResponse) {
    if (fhirResources.contains(groupName.dropLast(1))) {
      ...
      instructions.forEachIndexed { index, instruction -> mainNest.add(instruction) }
  ...
  fun Instruction.getAnswerExpression(questionnaireResponse: QuestionnaireResponse): String {
    ...
    // TODO: We should infer the resource element and add the correct conversion or code to assign
    // this correctly
    ...
    // 2. If the answer is from the QuestionnaireResponse, get the ID of the item in the
    // "Questionnaire Response Field Id" and
    // get its value using FHIR Path expressions
    if (responseFieldId != null) {
      // TODO: Fix the 1st param inside the evaluate expression
      var expression =
        "${"$"}this.item${getPropertyPath()}.where(linkId = '$responseFieldId').answer.value"
      // TODO: Fix these to use infer
      if (fieldPath == "id" || fieldPath == "rank") {
        expression =
          "create('${if (fieldPath == "id") "id" else "positiveInt"}') as $fieldPath, $fieldPath.value = evaluate(src, $expression)"
      } else {
    ...
    // 3. If it's a FHIR Path/StructureMap function, add the contents directly from here to the
    // StructureMap
    if (fhirPathStructureMapFunctions != null && fhirPathStructureMapFunctions!!.isNotEmpty()) {
      // TODO: Fix the 2nd param inside the evaluate expression --> Not sure what this is but check
      // this
  ...
  inner class Nest {
    ...
    fun add(instruction: Instruction) {
      ...
      remainingPath.run {
        if (contains(".")) {
          val parts = split(".")
          val partName = parts[0].ifEmpty { parts[1] }
          ...
          val newNest =
            Nest().apply {
              ...
              fullPath =
                if (this@Nest.fullPath.isNotEmpty()) {
              ...
              if (
                (parts[0].isEmpty() && parts.size > 2) || (parts[0].isNotEmpty() && parts.size > 1)
              ) {
                val nextInstruction =
                  Instruction().apply {
                    copyFrom(instruction)
    ...
    fun buildStructureMap(currLevel: Int, questionnaireResponse: QuestionnaireResponse) {
      if (instruction != null) {
          ...
          if (
            ...
            if (
              propertyType != "Type" &&
                ...
                propertyType?.canHandleConversion(answerType ?: "")?.not() == true &&
        ...
        if (!name.equals("")) {
          ...
          // stringBuilder.append("src -> entity$currLevel.$name = create('$resourceType') as
          // entity${currLevel + 1} then {")

        }

        nests.forEach { it.buildStructureMap(currLevel + 1, questionnaireResponse) }
...
fun StringBuilder.appendNewLine(count: Int = 1): StringBuilder {
  var counter = 0
  while (counter < count) {
...
internal val fhirPathEngine: FHIRPathEngine =
  with(FhirContext.forCached(FhirVersionEnum.R4)) {
    FHIRPathEngine(HapiWorkerContext(this, this.validationSupport)).apply {
      // TODO update to link to already existing FHIRPathEngineHostService in the external folder
