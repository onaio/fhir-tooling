...
public class QuestionnaireResponseGeneratorCommand implements Runnable{
    private static final Logger logger = Logger.getLogger(QuestionnaireResponseGeneratorCommand.class.getCanonicalName());
    ...
    private void generateResponse(String inputFilePath, String apiKey) throws IOException {
        ...
        questionnaireJsonString = questionnaireJsonString.substring(1, questionnaireJsonString.length()-1);

        // Generate response
        String generatedResponseString = aiGenerated(questionnaireJsonString, apiKey, this.aiModel, this.maxTokens);
        JSONObject obj = new JSONObject(generatedResponseString);
        JSONArray choices = obj.getJSONArray("choices");
        String questionnaireResponseString = "";
        for (int i=0; i < choices.length(); i++){
            questionnaireResponseString = choices.getJSONObject(i).getJSONObject("message").getString("content");
        }
        ...
        FctUtils.printInfo(
        String.format("Result Questionnaire Response generated and saved to path\u001b[36m %s \u001b[0m", output));

        FctUtils.printCompletedInDuration(start);
    }

    private static String aiGenerated(String questionnaireData, String apiKey, String model, String tokens) throws IOException{
        FctUtils.printInfo("Generating your questionnaire response");
        String endpoint = "https://api.openai.com/v1/chat/completions";
        String message = "Generate a fhir questionnaire response for the following fhir questionnaire: " + questionnaireData;
        String payload = "{\"model\": \"" + model + "\", \"messages\": [{\"role\": \"user\", \"content\": \"" + message + "\"}], \"max_tokens\": " + tokens + "}";
