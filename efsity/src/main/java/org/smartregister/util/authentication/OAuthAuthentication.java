    ...
    try {
              ...
              requestBody.keySet().stream()
                  .map(
                      key ->
                          key
                              + "="
                              + URLEncoder.encode(requestBody.get(key), StandardCharsets.UTF_8))
                  .collect(joining("&")));

      final byte[] clientCredentials =
          Base64.getEncoder()
      ...
      final HttpRequest request =
          HttpRequest.newBuilder()
              ...
              .header(
                  "Authorization", "Basic " + new String(clientCredentials, StandardCharsets.UTF_8))
